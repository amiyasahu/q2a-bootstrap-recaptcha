...
*/
	...
	class qa_boot_recaptcha_captcha {
		...
		function recaptcha_error_html()
		{
			...
			elseif ( (!strlen(trim(qa_opt('recaptcha_public_key')))) || (!strlen(trim(qa_opt('recaptcha_private_key')))) ) {
				...
				$url=recaptcha_get_signup_url(@$_SERVER['HTTPS_HOST'], qa_opt('site_title'));
		...
		function form_html(&$qa_content, $error)
		{
			...
			return recaptcha_get_html(qa_opt('recaptcha_public_key'), $error, true);
		}
